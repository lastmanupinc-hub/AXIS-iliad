...
- **Billing charges before the work can fail — no refund path.** `mcp-server.ts`: `meterMcpToolCredits` commits the debit *before* failure-prone steps (`runAnalyzeRepo` → `fetchGitHubRepo`; `runEmbeddings`/`runTransactionalEmail`/`runLlmInference` → the awaited provider call). Failed/timed-out calls silently cost the caller money. **Fix:** meter only after the success-determining step, charging on a non-error result. Also fix the allowance-boundary partial-charge (`mcp-server.ts:2501-2518`) and add **idempotency** on the paid path (retries currently double-charge). *(HIGH — real money)*
