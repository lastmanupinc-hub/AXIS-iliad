...
| # | Item | Blocks | Notes |
|---|---|---|---|
| P1 | ✅ **RESOLVED 2026-07-27** — `AXIS_TOKEN_KEY` | — | Was confirmed absent via Render's API (this is what broke GitHub login on 2026-07-26: the fail-closed throw landed *after* the account already existed and was swallowed into a generic error redirect). A 64-char random value is now installed and the service redeployed and healthy. Login had already been made independent of it (`9b3e2ea`); GitHub token storage for private repos is now unblocked too. **Not end-to-end proven** — confirming an actual token round-trip requires a real GitHub OAuth login through a browser. |
| P2 | ⏸ **DEFERRED by owner 2026-07-27** — `ALERT_WEBHOOK_URL` | All threshold alerting | **Accepted risk, not resolved.** Confirmed absent via Render's API; `startAlerting()` no-ops, so nothing pages on a 5xx spike. The only signal remains the 30-min synthetic filing a GitHub issue — which also cannot see a web-origin outage (that check is permanently suppressed, see §2). Owner deferred; revisit before real traffic. Unblocked by any webhook URL. |
| P3 | ✅ **RESOLVED 2026-07-27** — `ADMIN_API_KEY` | — | Was set on the service but absent from `render.yaml` — the same orphaning shape that left `RESEND_FROM_ADDRESS` one Blueprint sync from silently killing all email. Now declared with `sync: false` (`626d7b5`), so the Blueprint knows the key exists while the value stays dashboard-only. Admin endpoints were never actually down. |
