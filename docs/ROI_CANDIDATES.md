...
| # | Candidate | Tree position | Effort | Impact | Gates / dependencies | Confidence |
|---|---|---|---|---|---|---|
| 1.1 | **Recurring billing.** Today "Pro $99/month" is **a one-time charge** — PAI'D's checkout only supports `mode: "payment"` and 501s on `mode: "subscription"` (`packages/paid-client/src/index.ts:270-288`; `packages/mpp/src/index.ts:556-562`). Every "subscriber" pays once and keeps the tier forever. This is the single largest revenue gap in the platform. | Roots › Payment rails › PAI'D | L | **revenue** | `gated(external)` — PAI'D must ship recurring billing. Cross-repo; not fixable here. Interim: an honest renewal prompt, or Stripe-direct for recurring while PAI'D handles one-time. | **High** — documented in both clients |
...
| 1.3 | **Fix the lite-enforcement collection leak.** Recorded as revenue lever #1 in [[monetization-activation-workstream]]: collect-first at current prices before raising them. Lite caps *are* enforced pre-charge (`mcp-server.ts:272-284`), so re-verify whether this lever is still open before spending on it. | Branches › MCP › metering | M | **revenue** | None. Verify-then-act. | **Medium** — memory says open; code looks enforced. Re-audit first. |
...
| 3.4 | **Remove the unused 1 GB `/data` disk** from `render.yaml`. Nothing mounts it since the Neon migration — zero code references. | Roots › API hosting | S | enabling | `gated(owner)` — R5.4 requires confirming from the Render dashboard that the live disk is empty, which no read-only audit can see. Must land as one change set across `render.yaml`, `docker-compose.yml`, `deployment.test.ts` + 2 runbooks. | **High** |
| 3.5 | **Declare `autoDeploy` explicitly in `render.yaml`.** The key is absent; auto-deploy-on-push relies on a Render dashboard default. Given this repo's Blueprint-governance rule, an undeclared deploy trigger is exactly the fragility that bit `RESEND_FROM_ADDRESS`. | Roots › API hosting | S | enabling | None. | **High** — verified absent |
| 3.6 | **Make the Cloudflare deploy path unambiguous and loud.** Two deploy paths are documented (GH Actions wrangler vs Cloudflare Git-connect) and the Actions one is `continue-on-error: true` — a broken web deploy would not fail CI. | Roots › Web hosting | S | enabling, integrity | Owner must confirm which path is operative (dashboard). | **High** |
