...
| 3.11 | **The pricing page is advertised publicly but requires login.** `/pricing` sits in the API sitemap at priority 0.8 (`handlers.ts:3202`) and is explicitly `Allow:`ed in robots.txt (`:3152`), while the `plans` route is `authOnly` — so a search result for "Axis Iliad pricing" lands a prospect on a sign-in modal at the top of the funnel. Found by the new SEO guard test, which refuses to let an auth-gated page advertise itself. | Branches › Web › Plans | S | **revenue**, integrity | Owner call: almost certainly drop `authOnly` from the route (pricing pages are conventionally public), but making a gated page public is a product decision, not a cleanup. Alternative is removing `/pricing` from the sitemap and robots — which forfeits the funnel. | **High** — contradiction verified on all three surfaces |
