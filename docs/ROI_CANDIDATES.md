...
| # | Candidate | Tree position | Effort | Impact | Gates / dependencies | Confidence |
|---|---|---|---|---|---|---|
| 1.1 | **Recurring billing — ⚠ CONTRACT RECEIVED 2026-07-27; the remaining work is entirely OURS.** Today "Pro $99/month" is still **a one-time charge**: every "subscriber" pays once and keeps the tier forever. The contract blocker recorded here previously is **cleared**. PAI'D published the full subscription contract in reply to `TICKET-AXIS_TOOLBOX-subscription-contract-20260727` (PAI'D @ `925cb60d`), and it is verified by direct fetch of their live spec — not taken from their summary: **46** component schemas (was 38), now including `Plan`, `Subscription`, `BillingCycle`, `CreatePlanRequest`, `CreateSubscriptionRequest`, `CancelSubscriptionRequest`, `RecordChargeResultRequest` and `WebhookDelivery`, with typed `requestBody` + response on all 13 billing routes. Walkthrough: `payment processing/docs/SUBSCRIPTIONS_API.md`. Iliad still calls none of them — its only `/v1/subscriptions` calls go to **Stripe** (`apps/api/src/stripe.ts:277,618`). | Roots › Payment rails › PAI'D | L | **revenue** | **No longer gated on another team.** Three gates remain, all ours: **(1) Merchant feature flag.** `mode:"subscription"` is fully built on PAI'D's side — the 501 was `subscription_mode_unavailable`, meaning OUR account lacks the per-merchant `subscriptions_enabled` flag. One self-serve call (`POST /v1/merchant/feature-flags`), but note that endpoint is **not in PAI'D's published spec** — the body came from them directly, so it cannot be self-verified against the contract. **(2) Webhook correlation — OPEN**, `TICKET-AXIS_TOOLBOX-checkout-webhook-envelope-20260727`. Their `WebhookDelivery` documents `data` as the subject object with **no `.object` wrapper**, but covers only `subscription.*`, `billing_cycle.*` and `payment_intent.*` — not `checkout.session.*`, the only event we consume in production, where `paid-handlers.ts` reads `data.object.metadata`. Our fixtures hand-build `data.object`, so they assert the assumption back at us and cannot detect a mismatch. **(3) Plan lifecycle.** Plans are created `draft`; `POST /v1/plans/{id}/activate` is mandatory or subscription create fails with a bare `400 Plan is not active`. `POST /v1/subscriptions` returns **200, not 201**, on idempotent replay. Idempotency-Key is now honoured on all mutating billing routes (PAI'D fixed a real defect — previously a retried plan create silently duplicated). *Already landed:* event names corrected against their published enum, plus `subscription.renewed` and the deliberate `subscription.payment_failed` no-op. | **High** on the contract — fetched and parsed from the live spec 2026-07-27, independent of PAI'D's own summary. *Webhook envelope for `checkout.session.*`: unknown, ticket open, deliberately not guessed.* |
...
| 3.11 | ✅ **CLOSED 2026-07-27 (owner: "it is not blocked information")** — **The pricing page was advertised publicly but required login.** `/pricing` sits in the API sitemap at priority 0.8 (`handlers.ts:3202`) and is explicitly `Allow:`ed in robots.txt (`:3152`), while the `plans` route is `authOnly` — so a search result for "Axis Iliad pricing" lands a prospect on a sign-in modal at the top of the funnel. Found by the new SEO guard test, which refuses to let an auth-gated page advertise itself. | Branches › Web › Plans | S | **revenue**, integrity | Resolved by dropping `authOnly`. The gate withheld nothing — `GET /v1/plans` already served the same prices unauthenticated and the public Q&A + For-Agents pages already printed them — so it only bounced humans arriving from search. `PlansPage` was already written for anonymous viewing (`loggedIn` + `onRequireLogin`, prompting at plan SELECTION), so this activated existing code rather than adding behaviour. Owner declined an additional homepage pricing link. | **High** — verified on all three surfaces |
| 3.10 | ✅ **CLOSED** — **Per-page SEO metadata.** Every route shares one `<title>`/description/canonical; there is no helmet-equivalent. The new `/feedback` page ships `ContactPage` JSON-LD, but its `<title>` is the site-wide one. | Branches › Web | M | strategic | None — but adding a dependency needs discussion; a hand-rolled head manager does not. | **High** |
