...
| # | Candidate | Tree position | Effort | Impact | Gates / dependencies | Confidence |
|---|---|---|---|---|---|---|
| 1.1 | **Recurring billing — ⚠ GATED ON OUR OWN TERMS OF SERVICE, not on PAI'D.** Today "Pro $99/month" is **a one-time charge**: every "subscriber" pays once and keeps the tier forever. PAI'D published the full subscription contract (`TICKET-AXIS_TOOLBOX-subscription-contract-20260727`, PAI'D @ `925cb60d`), verified by direct fetch of their live spec: **46** component schemas (was 38) including `Plan`, `Subscription`, `BillingCycle` and `WebhookDelivery`, typed on all 13 billing routes. Walkthrough: `payment processing/docs/SUBSCRIPTIONS_API.md`. | Roots › Payment rails › PAI'D | L | **revenue** | **The blocker is legal, not technical, and it is ours.** **(1) TermsPage §4.3 forbids it.** The published Terms promise "a single, one-time charge", "We do not store your payment method", and "a purchase does not automatically renew or re-charge you". `mode:"subscription"` vaults the card, auto-renews and re-charges — breaking all three. Shipping it without changing the Terms first would bill customers contrary to the contract they accepted. **Sequence the ToS change first; this is an owner decision, not an engineering one.** **(2) Then two code changes, not the "one line" PAI'D's correction claimed.** `mode` must become `"subscription"` AND the session needs a cadence — one of `plan_id`, a priced `price_id`, or, for the `ad_hoc` line item we send, `recurring: {interval: "month" or "year"}` (map `input.cycle` monthly→month / annual→year). Omitting the cadence is a `400`, distinct from the `501` that means flag-off. **Corrected — NOT a gate:** the `subscriptions_enabled` flag. PAI'D's first reply said we had to set it; PAI'D retracted that the same day — it has been `enabled:true, rollout_percent:100` since **2026-07-08**, three weeks before we filed. Nothing was ever waiting on it. **Also not a gate:** webhook handling, already correct (tier on `subscription.created`/`renewed`, deactivate only on `canceled`, `payment_failed` a deliberate no-op). | **High** on the contract (fetched from the live spec) and on the ToS conflict (read directly from `TermsPage.tsx` §4.3, quoted above). *Unverified: whether the owner intends to change the Terms — that decision has not been made.* |
