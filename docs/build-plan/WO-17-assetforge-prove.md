...
**CREDENTIAL SOURCE (resolved 2026-07-07 -- use this method when WO-17 is picked up, do not re-derive):** owner-authorized reuse of Foundry's existing `HF_TOKEN`, already live on the `axis-avatar-api` Render service (`srv-d72937cg9agc7392q610`). Verified via HF's `whoami-v2`: a valid **fine-grained** token named `"foundry"` (created 2026-05-27), `canReadGatedRepos: true`, explicitly scoped to `black-forest-labs/FLUX.2-dev` (one of this WO's own target Spaces) and `facebook/dinov3-vitl16-pretrain-lvd1689m`, plus broad `inference.*` grants on the owner's own account namespace. Fine-grained repo scoping governs *repo content* operations (weight downloads/pushes); calling a public Space's Gradio interface via `gradio_client.Client(space_id, hf_token=...)` only needs a valid HF identity for ZeroGPU/rate-limit queue priority, not per-repo permission -- so this token is expected to work for all of Microsoft/TRELLIS, GuyTevet/MotionDiffusion, and the eventual SDXL/FLUX Space, not just the one it's explicitly scoped to. **Fetch method (mirrors the Stripe/Resend credential pattern used elsewhere in this program):** read `RENDER_API_KEY` from `key.txt`, `GET https://api.render.com/v1/services/srv-d72937cg9agc7392q610/env-vars`, extract the `HF_TOKEN` entry -- use it in-process for local dev/test only, never print/commit it, never push it into the public `assetforge` repo (source, CI config, or otherwise) or its GitHub Secrets without a separate explicit go-ahead, since that repo is a different trust boundary than this one. **What this does NOT solve:** ZeroGPU quota sufficiency for TRELLIS, the guessed/unvalidated Gradio API signatures (still `view_api()`-resolve live per the Target State below), Space uptime, or the SDXL/FLUX model-vs-Space-ID mismatch -- the token only removes "no credential to test against" as a blocker; it does not make the guessed API calls correct.
...
- ~~An HF_TOKEN...~~ **RESOLVED** -- see "CREDENTIAL SOURCE" above (reuse Foundry's `axis-avatar-api` `HF_TOKEN`). ZeroGPU quota sufficiency under that token is still unverified and remains a real gate.
- CI's `live-canary` job needs `HF_TOKEN` as a GitHub Actions secret on the `assetforge` repo specifically -- setting that is a separate, owner-only step (a new secret on a different repo than this one), distinct from the local dev-loop credential fetch above.
