...
**CREDENTIAL SOURCE:** resolved 2026-07-07, owner-authorized reuse of an existing Foundry
token with the scope this WO needs (details in the owner's private ops runbook, not this
public repo — this doc previously named the exact Render service and a step-by-step
extraction recipe, which was public-repo-inappropriate regardless of the "never
print/commit" caveat attached to it; redacted 2026-09-02). Anyone picking up WO-17
should ask the owner for the credential directly rather than re-deriving a fetch path.
**What this does NOT solve:** ZeroGPU quota sufficiency for TRELLIS, the guessed/unvalidated
Gradio API signatures (still `view_api()`-resolve live per the Target State below), Space
uptime, or the SDXL/FLUX model-vs-Space-ID mismatch -- the credential only removes "no
credential to test against" as a blocker; it does not make the guessed API calls correct.
