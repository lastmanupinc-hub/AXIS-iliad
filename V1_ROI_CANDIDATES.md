...
**Result (2026-07-19, commit `eb8d551` on main).** All 3 steps built. `eslint.config.js` now combines `@eslint/js` recommended + `typescript-eslint` recommended-type-checked (`projectService: true` — the modern equivalent of hand-listing all 13 package tsconfigs) with `no-floating-promises`/`no-misused-promises` as errors, `no-unused-vars` as a warning. Root `"lint": "eslint ."`; `ci.yml` runs it right after `Type-check`.

The previously-unknown baseline: **33 `no-floating-promises`/`no-misused-promises` findings across 13 files** — all hand-fixed, none suppressed (small enough per step 2's own branching, and explicitly in-scope as real bugs). Most were an async handler passed directly to a void-expecting prop (`onClick`/`onSubmit`/`onChange`/`onDrop`), fixed via the `() => void handler()` wrapper. Two were genuine unhandled-rejection crash risks: `router.ts`'s `createServer` callback and its `res.on("finish", async …)` listener could take the **entire server** down on a single bad request, since this file's own `unhandledRejection` handler responds to any uncaught rejection by shutting the whole process down — now isolated per-request behind a `try/catch` + 500 response. Also fixed while baselining: 4 pre-existing syntax errors (an unescaped apostrophe in "Axis' Iliad" prematurely closing a single-quoted string, in `generative-sketch.js` + 3 `scripts/*.js` posting scripts) that `tsc` could never catch since these are plain `.js` — concrete proof of this gate's value independent of the existing typecheck gate.

The remaining ~312 findings (`no-unsafe-assignment`, `no-unnecessary-type-assertion`, `require-await`, etc. — real type-safety debt, surfaced by turning on the full `recommended-type-checked` preset repo-wide for the first time) were too large for this one unit. Landed as an advisory baseline via ESLint's own **suppressions** feature (`eslint-suppressions.json`, committed) — fails CI only on a NEW finding past today's baseline, never a flag-day red build. Run `eslint --prune-suppressions` after fixing a suppressed finding to keep the baseline honest as debt is paid down.

Verified in the repo's own order: `tsc -b` clean → a deliberately seeded floating promise failed `pnpm lint`, removing it passed again → `counts-consistency`/`count-honesty`/`launch-claims`/`strategic-docs-honesty` (31 tests) untouched → full monorepo build green → CI green on both Node matrix jobs (the new Lint step included) plus secret-scan/deploy-web/live-probe.
