...
| A6 | mcp-server-decompose | Carve the god-file + break the `handlers.ts ↔ mcp-server.ts` circular import. **Done:** circular import broken (intent/probe telemetry → new `intent.ts`; handlers imports from there, not mcp-server); `MCP_TOOLS` catalog (1,414-line literal) + `toolAnnotations` + 3 schemas + the planned-capability machinery → new `mcp-tools.ts` (re-exported so importers keep working). **mcp-server.ts 5,261 → 3,700 LOC (−30%).** Remaining (follow-up, A6b): extract the ~20 `run*` tool impls + the credit/billing block to reach <1.5k. | MED | 40 | 🟡 **~55%** · branch `feat/audit-a6-decompose`. No circular import; no new cycle (mcp-tools/intent import only counts.js). 226 tests green (full dispatch suite + counts + prepare) on real pg16; tsc clean. |
| A7 | sqlite-layer-removal | Deleted the 877-line dead SQLite `db.ts` (production-dead post-Neon; only its own tests + an obsolete bench used it) + its SQLite-era tests (`db.test.ts`, `db-maintenance.test.ts`) + the broken `perf.bench.ts` (mixed SQLite setup w/ pg stores). Removed the package-index re-export; dropped `better-sqlite3` + `@types/better-sqlite3` (snapshots + api). Converted the one live consumer — `email-store.test.ts`'s schema check — from a SQLite `pragma` to a pg `information_schema` query. | MED | 38 | ✅ **100%** · branch `feat/audit-a7-sqlite-removal`. No remaining `better-sqlite3`/`db.js` imports; tsc clean (snapshots + api); email-store + store 54/54 on real pg16; full snapshots suite green. |
...
| A12 | stale-stack-docs | README has a DUPLICATED "Stack" section with contradictory test counts (1485/68 vs 4076/140 — real is ~4,970/193) and a stale "SQLite (better-sqlite3, WAL)" line (the data layer is Neon Postgres now). Also verify the "14 MCP tools" claim against the real MCP_TOOLS registry. Found while doing A4. | LOW | 22 | 0% · proof: one stack section, accurate test count + Neon, verified tool count. |

**Build order (risk-ranked):** ✅ A1 → ✅ A2 → ✅ A5 → ✅ A3 → ✅ A4 → ✅ A11 → 🟡 A6 (~55%; run\*-impl carve = follow-up A6b) → ✅ A7 → **A9 (next)** → A8 → A10 → A12.
