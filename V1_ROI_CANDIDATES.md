...
| A11 | flaky-test-stabilization | `crash-resilience` (keep-alive/process-handlers) and `iliad_code_sandbox` (Docker) tests intermittently red the CI on main from timing/port/Docker flakiness — masking real failures. Make them deterministic (or quarantine + ticket). Found while landing A1/A2 (A1 CI green, A2 CI red purely on the crash-resilience flake; 4945/4946 passed). | MED | 35 | 0% · proof: 10× consecutive green CI runs; no timing-based sleeps in the two suites. |
...
**Build order (risk-ranked):** ✅ A1 → ✅ A2 → ✅ A5 → ✅ A3 → ✅ A4 → **A11 (next)** → A6 → A7 → A8 → A9 → A10 → A12.
