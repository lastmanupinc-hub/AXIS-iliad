...
axis-iliad is a monorepo built with TypeScript using React. It contains 500 files across 15 top-level directories. It defines 446 domain models.
...
## Compliance-Engine Performance (measured)

The seven agentic-commerce engines (detectCommerceSignals, computeComplianceGrade, generateAgentPurchasingPlaybook, generateProductSchema, generateCheckoutFlow, generateNegotiationRules, generateCommerceRegistry) are pure, synchronous, in-memory functions:

- **External API calls at runtime: 0** — runtime-proven by `packages/generator-core/src/perf-compliance.test.ts` (a throwing `globalThis.fetch` spy is installed and never fires). PCI scope: none — no network or cardholder data is reached.
- **Latency: measured, not zero** — the current measured p50 lives in `packages/generator-core/perf-results.json` (low single-digit ms in-process; regenerate via `pnpm bench:compliance` or the vitest suite). Do not cite "0ms".
- **No head-to-head vs Visa IC** — the Visa Intelligent Commerce 200-800ms figure is a published industry range, NOT measured by this harness; docs must attribute it as such or drop the comparator (enforced by the doc-honesty test in perf-compliance.test.ts).
