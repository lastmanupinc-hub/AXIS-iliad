...
| R4.3 | Archive | BLOCKED | *(missing from this table at authoring time -- added 2026-07-26; the unit body at line ~502 and cross-refs from R0.1's and R4.1's own write-ups always described it as real work, just never got a cursor row)*. Attempted, reverted -- see FAILURES ledger. Blocked on the newly-found scanner.ts breadth-starvation bug in the FINDINGS queue, not on anything this unit itself controls. |
...
|---|---|---|
| `apps/cli/src/scanner.ts` | R0.3's files-before-dirs fix (this session) solved root-file starvation but not **sibling-top-level-directory** starvation: `walk()` still recurses fully depth-first into the first alphabetical subdirectory before ever touching its siblings, so one large top-level dir (`apps/`, well over 100 files once `apps/api`+`apps/web`+`apps/cli` are all walked) can alone exhaust `MAX_FILES=500` before the walk ever reaches sibling top-level dirs (`docs/`, `examples/`, `mcp/`, `packages/`, `search/`, ...). Confirmed empirically running R4.3's dogfood just now: this repo has ~10 real top-level dirs but the regenerated `.ai/CLAUDE.md` reported only 3. Proper fix is breadth-first (queue-based level-order) traversal instead of the current depth-first recursion, so every top-level dir gets at least shallow representation before any one of them goes deep -- not attempted this session: `scanner.ts` is shared by every analysis this tool runs (not just self-dogfood), a BFS rewrite needs its own careful test pass, and a Node segfault had just fired on this machine (see R4.3's FAILURES entry) -- not the moment to rush a rewrite of core, widely-used traversal code. | Regenerated `.ai/` reverted rather than committed (`git checkout -- .ai/ AGENTS.md CLAUDE.md .cursorrules` + removed 2 new stray files) rather than commit a narrower, lower-quality self-dogfood snapshot than what's already live |
