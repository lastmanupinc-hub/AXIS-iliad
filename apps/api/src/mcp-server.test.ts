...
describe("POST /mcp — tools/list", () => {
  it("returns the full 35-tool catalog (build-not-redact catalog honesty)", async () => {
...
describe("getMcpServerMeta — shape and content", () => {
  ...
  it("tools array exposes the full 35-tool catalog (build-not-redact)", async () => {
...
describe("GET /v1/mcp/server.json", () => {
  ...
  it("body contains 35 tools (full catalog, build-not-redact; image_generation delegated to AXIS Foundry sibling)", async () => {
    const r = await get("/v1/mcp/server.json");
    const data = r.data as Record<string, unknown>;
...
describe("POST /mcp — tools/call discover_commerce_tools", () => {
  it("returns tool overview with install configs (no auth required)", async () => {
    ...
    expect(parsed.tools.length).toBe(35);
  });
