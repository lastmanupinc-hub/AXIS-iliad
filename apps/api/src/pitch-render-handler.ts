...
// one xAI call per distinct slide `art` key (at most 5 for the current 5-slide
// deck shape) and NEVER fails the whole request if xAI is unconfigured or a
// call errors — those slides just get the clean solid-color fallback
...
/** slide-art-prompts.json's own key set (generators-pitch.ts's buildSlides) — bounds the worst-case xAI call count per render. */
const MAX_BACKGROUND_CALLS = 8;
...
export async function handlePitchRender(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const rendered = await renderPitchDeckPptx(deck, { backgrounds });

  res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.presentationml.presentation");
  res.setHeader("Content-Disposition", `attachment; filename="${(deck.project ?? "pitch-deck").replace(/[^A-Za-z0-9._-]/g, "_")}.pptx"`);
