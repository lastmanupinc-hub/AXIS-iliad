...
/**
 * The "here's how to actually get this processed" half of an oversized-upload
 * 413. Factored out because `handleAnalyze` carried it and its
 * `handleCreateSnapshot` twin did not — the same REST/twin divergence this
 * codebase keeps rediscovering — so a caller hitting the cap on
 * `POST /v1/snapshots` got a bare "limit exceeded" with no way forward.
 *
 * Deliberately does NOT suggest "use the other endpoint instead": both gate
 * anonymous callers on the identical `TIER_LIMITS.free` caps, so pointing a
 * 413'd caller from one to the other would be a false promise that wastes a
 * retry. The only real remedies are fewer/smaller files or a higher tier, and
 * this says exactly that.
 *
 * Returns undefined when no tier would fit (already at the top, or the upload
...
function oversizedUploadGuidance(
  ...
  const accommodatingTier = findAccommodatingTier(files, currentTier);
  if (!accommodatingTier) return undefined;
  const limits = TIER_LIMITS[accommodatingTier];
  return {
...
const ADOPTION_HINTS: Record<string, { placement: string; adoption_hint: string }> = {
  ...
  "pitch-deck.md":            { placement: "root", adoption_hint: "The truth-first deck (annotated diligence copy) on the evidence-based consensus skeleton. Every figure is measured; owner-input slots are labeled, never invented; docs numbers appear only on the honest-audit slide as audited claims. Render the clean investor .pptx via POST /v1/pitch/render." },
  ...
  "slide-art-prompts.json":   { placement: ".ai/", adoption_hint: "Per-slide background prompts. Render via POST /v1/pitch/render with render_backgrounds:true (xAI) or scripts/pitch-backgrounds.mjs." },
  ...
  sendJSON(res, 200, {
    ...
    skills: [
      ...
      },
      {
        ...
        endpoint: "POST /mcp",
        auth_required: false,
        // H-Phase-A cycle 11: was a hand-typed 21-entry array, missing 16 real
        // tools (the same hand-duplicated-catalog-drift shape as capabilities.json's
        // mcp.tools, fixed cycle 10) -- derived from the same
        // deriveMcpToolCatalog() source so it can't independently drift again.
        tools: deriveMcpToolCatalog().map((t) => t.name),
      },
    ],
  });
...
- ${MCP_TOOL_COUNT} tools: ${deriveMcpToolCatalog().map((t) => t.name).join(", ")}
