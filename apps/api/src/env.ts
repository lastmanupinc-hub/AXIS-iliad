...
export const ENV_SPEC: EnvSpec[] = [
  ...
  { key: "LINKEDIN_CLIENT_ID", required: false, type: "string", description: "LinkedIn OAuth 2.0 client ID (Sign In with LinkedIn using OpenID Connect) for web login (GET /v1/auth/linkedin)." },
  { key: "LINKEDIN_CLIENT_SECRET", required: false, type: "string", description: "LinkedIn OAuth 2.0 client secret. Required alongside LINKEDIN_CLIENT_ID for the callback token exchange." },
  { key: "LINKEDIN_CALLBACK_URL", required: false, type: "string", default: "http://localhost:4000/v1/auth/linkedin/callback", description: "LinkedIn OAuth redirect URI. Must byte-match an Authorized redirect URL registered on the LinkedIn Developer App, or the token exchange fails." },
  { key: "AXIS_OWNER_EMAIL", required: false, type: "string", description: "When set, a successful login (any OAuth provider or pasted key) resolving to an account with this exact email also grants the HttpOnly admin-elevation cookie (see ADMIN_COOKIE) — no separate /v1/admin/session key entry needed. Case-insensitive compare. No effect unless ADMIN_API_KEY is also set." },
