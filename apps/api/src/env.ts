...
export const ENV_SPEC: EnvSpec[] = [
  ...
  { key: "AXIS_OWNER_EMAIL", required: false, type: "string", description: "When set, a successful login (any OAuth provider — e.g. GitHub — or a pasted key) resolving to an account with this exact email also grants the HttpOnly admin-elevation cookie (see ADMIN_COOKIE) — no separate /v1/admin/session key entry needed. Case-insensitive compare. No effect unless ADMIN_API_KEY is also set." },
