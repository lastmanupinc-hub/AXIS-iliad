...
import { getSnapshot, isProgramEnabled, isFreeTrialActive, recordUsage } from "@axis/snapshots";
...
/**
 * Same reason this file duplicates assertSnapshotAccess rather than importing
 * it: a tiny, stable best-effort wrapper, not worth the handlers.ts coupling.
 * Closes a real cross-account-analytics gap — this endpoint had no
 * usage_records row at all, so it was invisible to getRestUsageSummary's
 * per-program reporting. No files are generated by an answer, so
 * generators_run/input_files/input_bytes are all 0.
 */
async function recordNotebookAskUsage(accountId: string, snapshotId: string): Promise<void> {
  try {
    await recordUsage(accountId, "notebook_ask", snapshotId, 0, 0, 0);
  } catch (err) {
    log("error", "record_usage_failed", {
      account_id: accountId,
      program: "notebook_ask",
...
export async function handleNotebookAsk(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  await recordNotebookAskUsage(auth.account.account_id, snapshotId);
  sendJSON(res, 200, { ...result, trial: buildTrialNotice(false) });
}
