...
Detected 14 component file(s). Apply these patterns:
...
- `/health` (GET) → docs/archive/e2e_ui_audit.yaml
- `/v1/health` (GET) → docs/archive/e2e_ui_audit.yaml
- `/v1/accounts` (POST) → apps/api/src/server.ts
- `/v1/account` (GET) → apps/api/src/server.ts
- `/v1/account` (PATCH) → apps/api/src/server.ts
- `/v1/account` (DELETE) → apps/api/src/server.ts
...
- **You are here:** `theme-guidelines.md` — agent step 14 of 71.
