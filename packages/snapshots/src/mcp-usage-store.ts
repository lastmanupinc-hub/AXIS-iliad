 ...
 *
 * Uses peekPool() so it NEVER lazily opens the pool — if no pool is open it is a
 * silent no-op. Callers on the hot request path should still wrap this in
 * try/catch: persistence telemetry must never break or slow a tool call.
 */
export async function recordMcpUsage(input: McpUsageInput): Promise<void> {
  if (!peekPool()) return;
  await sql.run(
...
export async function getMcpUsageWindows(): Promise<McpUsageWindows> {
  ...
  return {
    ...
    last_7d: await count("SELECT COUNT(*) c FROM mcp_usage WHERE created_at >= ?", sinceIso(7)),
    last_30d: await count("SELECT COUNT(*) c FROM mcp_usage WHERE created_at >= ?", sinceIso(30)),
  };
}

/** Aggregate breakdown of calls in the trailing window (default 30 days). */
export async function getMcpUsageSummary(options?: { windowDays?: number }): Promise<McpUsageSummary> {
  ...
  return {
    ...
    total_calls: await scalar("SELECT COUNT(*) c FROM mcp_usage WHERE created_at >= ?"),
    unique_accounts: await scalar(
      "SELECT COUNT(DISTINCT account_id) c FROM mcp_usage WHERE created_at >= ? AND account_id IS NOT NULL",
    ),
    anonymous_calls: await scalar(
      "SELECT COUNT(*) c FROM mcp_usage WHERE created_at >= ? AND account_id IS NULL",
    ),
    by_tool: await groupBy("tool"),
    by_source: await groupBy("source"),
    by_probe_class: await groupBy("probe_class"),
  };
}
 ...
 * (no stable identity).
 */
export async function getMcpUsageNewVsReturning(options?: {
  windowDays?: number;
}): Promise<McpNewVsReturning> {
  const windowDays = options?.windowDays ?? 30;
  const since = sinceIso(windowDays);
  const rows = await sql.many<{ account_id: string; first_seen: string; active_in_window: number }>(
    `SELECT account_id,
  ...
  for (const r of rows) {
    if (Number(r.active_in_window) !== 1) continue;
...
export async function getRecentMcpUsage(limit = 50): Promise<McpUsageRow[]> {
  return await sql.many<McpUsageRow>(
    "SELECT * FROM mcp_usage ORDER BY created_at DESC LIMIT ?",
    [limit],
  );
}
