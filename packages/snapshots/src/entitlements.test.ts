...
describe("entitlements", () => {
  ...
  it("an account has no entitlements until granted one", async () => {
    ...
    expect(await hasEntitlement(account.account_id, "theme")).toBe(false);
  ...
  it("grantEntitlement makes hasEntitlement true for that product only", async () => {
    ...
    await grantEntitlement(account.account_id, "theme");
    expect(await hasEntitlement(account.account_id, "theme")).toBe(true);
    expect(await hasEntitlement(account.account_id, "canvas")).toBe(false);
  });
  ...
  it("is idempotent — granting the same product twice does not duplicate or error", async () => {
    ...
    await grantEntitlement(account.account_id, "deploy");
    await grantEntitlement(account.account_id, "deploy");
    const rows = await listEntitlements(account.account_id);
    expect(rows.filter((r) => r.product_id === "deploy").length).toBe(1);
  });
  ...
  it("scopes entitlement to the account it was granted to, not every account", async () => {
    ...
    await grantEntitlement(a.account_id, "artifacts");
    expect(await hasEntitlement(a.account_id, "artifacts")).toBe(true);
    expect(await hasEntitlement(b.account_id, "artifacts")).toBe(false);
  });

  it("records the granting source and a real timestamp", async () => {
    const account = await createAccount("source", "source@test.com", "paid");
    await grantEntitlement(account.account_id, "closer", "manual");
  ...
  it("defaults source to 'purchase' when not specified", async () => {
    ...
    await grantEntitlement(account.account_id, "marketing");
    const [row] = await listEntitlements(account.account_id);
  ...
  it("lists multiple entitlements ordered oldest-granted first", async () => {
    ...
    await grantEntitlement(account.account_id, "theme");
    await grantEntitlement(account.account_id, "canvas");
    const rows = await listEntitlements(account.account_id);
    expect(rows.map((r) => r.product_id)).toEqual(["theme", "canvas"]);
  });
});
