// Adapted from the AXIS CLI (apps/cli/src/runner.ts) — manifest, framework,
// and project-type detection only. Imports rewired to the local snapshots types.

import type { FileEntry, SnapshotManifest } from "../snapshots/types.js";

/** Auto-detect project metadata from scanned files */
export function detectManifest(files: FileEntry[], projectDir: string): SnapshotManifest {
