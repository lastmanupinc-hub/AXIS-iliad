...
// ─── vault notes: the actual vault, not instructions to build one ─────────
//
// app_35_obsidian_vault_sync: everything above tells a human HOW to build a
// vault (templates, linking policy, a graph "prompt map"). This is the vault
// itself — one real note per source file that participates in this repo's
// OWN import graph, cross-linked via ctx.dependency_graph.internal_imports
// (the SAME real, already-computed edge data generateArchitectureDiagram
// uses for canvas — no new extraction logic, no fabricated relationships).
//
// codeFileNote() above is reused for note identity, exactly per its own
// docstring's intent ("shared by every generator so the same file has a
// single note identity across all outputs").
//
// Deliberately excludes files with NO import relationship: a vault of
// isolated, unlinked notes is worse than no vault (Obsidian's whole value is
// the graph), and "0 edges" almost always means the file was never scanned
// for imports rather than a genuine island — see generateArchitectureDiagram's
// identical judgment call for canvas.

/** Real per-file cross-reference edges from the repo's own import graph. */
function fileEdges(ctx: ContextMap): { outgoing: Map<string, string[]>; incoming: Map<string, string[]> } {
  const outgoing = new Map<string, string[]>();
  const incoming = new Map<string, string[]>();
  for (const imp of ctx.dependency_graph.internal_imports) {
    if (imp.source === imp.target) continue; // self-edge — nothing to link
    (outgoing.get(imp.source) ?? outgoing.set(imp.source, []).get(imp.source)!).push(imp.target);
    (incoming.get(imp.target) ?? incoming.set(imp.target, []).get(imp.target)!).push(imp.source);
  }
  return { outgoing, incoming };
}

/** One markdown note per file that has at least one real import edge. Path: `vault/<note>.md`. */
export function generateVaultNotes(ctx: ContextMap, _files?: SourceFile[]): GeneratedFile[] {
  const { outgoing, incoming } = fileEdges(ctx);
  const paths = new Set<string>([...outgoing.keys(), ...incoming.keys()]);

  const notes: GeneratedFile[] = [];
  for (const path of [...paths].sort()) {
    const out = [...new Set(outgoing.get(path) ?? [])].sort();
    const inc = [...new Set(incoming.get(path) ?? [])].sort();

    const lines: string[] = [];
    lines.push(`# ${mdInline(path)}`);
    lines.push("");
    lines.push(`> Generated by AXIS obsidian (watch mechanic). Links reflect this repo's actual import graph — nothing fabricated.`);
    lines.push("");
    if (out.length > 0) {
      lines.push("## Imports");
      for (const t of out) lines.push(`- [[${codeFileNote(t)}]] (\`${mdInline(t)}\`)`);
      lines.push("");
    }
    if (inc.length > 0) {
      lines.push("## Imported by");
      for (const s of inc) lines.push(`- [[${codeFileNote(s)}]] (\`${mdInline(s)}\`)`);
      lines.push("");
    }

    notes.push({
      path: `vault/${codeFileNote(path)}.md`,
      content: lines.join("\n"),
      content_type: "text/markdown",
      program: "obsidian",
      description: `Vault note for ${path} — real import links, not templated`,
    });
  }
  return notes;
}
...
/**
 * The candidate's actual hard requirement, made checkable: every [[wikilink]]
 * in every generated note must resolve to a note IN THIS SAME SET. A broken
 * link is a real defect — the customer will click it in Obsidian and land
 * nowhere — so this is the enforcement point, not a nice-to-have.
 */
export function verifyVaultLinks(notes: GeneratedFile[]): { broken: Array<{ note: string; link: string }> } {
  const knownBasenames = new Set(
    notes.map((n) => n.path.replace(/^vault\//, "").replace(/\.md$/, "")),
  );
  const broken: Array<{ note: string; link: string }> = [];
  const linkRe = /\[\[([^\]]+)\]\]/g;
  for (const note of notes) {
    for (const m of note.content.matchAll(linkRe)) {
      if (!knownBasenames.has(m[1])) broken.push({ note: note.path, link: m[1] });
    }
  }
  return { broken };
}
