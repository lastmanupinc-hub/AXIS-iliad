...
// D2 (https://d2lang.com, MPL-2.0) diagram source — app_24_canvas_rendered_diagrams.
// Real edges only: every arrow comes from an actual cross-directory import
// detected by repo-parser (ctx.dependency_graph.internal_imports), grouped by
// each file's top-level directory (ctx.structure.top_level_layout) — the same
// grouping repo-parser itself already computes, not a fresh guess. No
// fabricated boxes: a directory with no real inbound/outbound edges to
// another top-level directory doesn't appear as a node at all.

function d2Id(name: string): string {
  // D2 keys need quoting if they contain characters outside [a-zA-Z0-9_] —
  // simplest correct approach is to always quote and escape embedded quotes.
  return `"${name.replace(/"/g, '\\"')}"`;
}

/** Builds real inter-directory edges from internal_imports, grouped by each file's top-level directory, deduped and counted, self-loops dropped (a directory importing its own files isn't cross-module architecture). */
function computeDirectoryEdges(ctx: ContextMap): Array<{ from: string; to: string; count: number }> {
  ...
  for (const imp of ctx.dependency_graph.internal_imports) {
    const from = dirOf(imp.source);
    const to = dirOf(imp.target);
    if (from === to) continue; // same top-level dir — not a cross-module edge
    const key = `${from} ${to}`;
...
export function generateArchitectureDiagram(ctx: ContextMap, _profile: RepoProfile, files?: SourceFile[]): GeneratedFile {
  const id = ctx.project_identity;
  const edges = computeDirectoryEdges(ctx).slice(0, 30); // cap: keep the diagram legible, strongest real signals first

  const dirLabels = new Map(ctx.structure.top_level_layout.map((d) => [d.name, d]));
  const nodesInDiagram = new Set<string>();
  for (const e of edges) {
    nodesInDiagram.add(e.from);
    nodesInDiagram.add(e.to);
  }

  const lines: string[] = [];
  lines.push(`# ${id.name} — architecture diagram`);
  lines.push(`# Generated by AXIS Canvas. Nodes are top-level directories;`);
  lines.push(`# edges are REAL cross-directory imports detected in this repo — nothing fabricated.`);
  lines.push("");

  if (nodesInDiagram.size === 0) {
    lines.push(`# No cross-directory imports detected — ${files?.length ?? 0} files scanned.`);
    lines.push(`# Either this project has a flat layout, or its modules don't import across top-level directories.`);
    lines.push(`empty: "No cross-module edges detected"`);
  } else {
    for (const name of [...nodesInDiagram].sort()) {
      const dir = dirLabels.get(name);
      const label = dir ? `${name} (${dir.file_count} files)` : name;
      lines.push(`${d2Id(name)}: ${JSON.stringify(label)}`);
    }
    lines.push("");
    for (const e of edges) {
      lines.push(`${d2Id(e.from)} -> ${d2Id(e.to)}: "${e.count}"`);
    }
  }

  return {
    path: "architecture-diagram.d2",
    content: `${lines.join("\n")}\n`,
    content_type: "text/vnd.d2",
    program: "canvas",
    description: "D2 diagram source for this repo's real inter-module dependency structure — render with the d2 CLI (https://d2lang.com) or via AXIS's watch mechanic.",
  };
}
