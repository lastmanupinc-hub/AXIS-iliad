...
export function generateIncidentTemplate(ctx: ContextMap, files?: SourceFile[]): GeneratedFile {
  const id = ctx.project_identity;
  const lines: string[] = [];
  ...
  if (files && files.length > 0) {
    const hotspotPaths = ctx.dependency_graph.hotspots
      ...
      .slice(0, 3)
      .map(h => h.path);
    const suspectFiles = files.filter(f => hotspotPaths.some(hp => f.path.endsWith(hp) || f.path.includes(hp)));
    if (suspectFiles.length > 0) {
      lines.push(...renderExcerpts("Suspect File Excerpts", suspectFiles, 25));
    }

    const entries = findEntryPoints(files);
    if (entries.length > 0) {
      lines.push(...renderExcerpts("Entry Point Excerpts", entries.slice(0, 3), 20));
    }
  }
...
export function generateTracingRules(ctx: ContextMap, files?: SourceFile[]): GeneratedFile {
  const id = ctx.project_identity;
  const lines: string[] = [];
  ...
  if (files && files.length > 0) {
    const entries = findEntryPoints(files);
    if (entries.length > 0) {
      lines.push("## Trace-Ready Entry Points");
      lines.push("");
    ...
    if (hotspotFiles.length > 0) {
      lines.push(...renderExcerpts("Hotspot Files to Instrument", hotspotFiles, 20));
    }
  }
...
export function generateRootCauseChecklist(ctx: ContextMap, files?: SourceFile[]): GeneratedFile {
  const id = ctx.project_identity;
  ...
  if (files && files.length > 0) {
    ...
    const hotspotFiles = files.filter(f => hotspotPaths.some(hp => f.path.endsWith(hp) || f.path.includes(hp)));
    if (hotspotFiles.length > 0) {
      lines.push("## Suspect File Excerpts (for Step 5)");
      lines.push("");
      ...
      lines.push(...renderExcerpts("Suspect File Source", hotspotFiles, 25));
    }
  }
