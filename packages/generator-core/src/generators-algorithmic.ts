...
export function generateGenerativeSketch(ctx: ContextMap, files?: SourceFile[]): GeneratedFile {
  ...
  lines.push("/**");
  lines.push(` * Generative Sketch — ${codeComment(id.name)}`);
...
export function generateCollectionMap(ctx: ContextMap, files?: SourceFile[]): GeneratedFile {
  ...
  lines.push(`# Collection Map — ${mdText(id.name)}`);
  lines.push("");
  ...
  lines.push(`metrics, and architecture of ${mdText(id.name)}.`);
  lines.push("");
  ...
  if (hotspots.length > 0) {
    ...
    for (const h of hotspots.slice(0, 5)) {
      lines.push(`  - \`${mdCode(h.path)}\` (risk: ${h.risk_score.toFixed(1)}, connections: ${h.inbound_count + h.outbound_count})`);
    }
  }
