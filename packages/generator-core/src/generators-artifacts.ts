...
export function generateComponent(ctx: ContextMap, files?: SourceFile[]): GeneratedFile {
  ...
  if (isSvelte) {
    lines.push(`<!-- ${codeComment(id.name)} — Svelte app shell. Renders a header, error boundary,`);
    lines.push(`     and a grid of detected routes / domain models. Pair with /theme.css. -->`);
    lines.push(`<script lang="ts">`);
    lines.push(`  let title = "${jsString(id.name)}";`);
    lines.push(`  let routes = ${JSON.stringify(routes.slice(0, 12).map(r => `${r.method.toUpperCase()} ${r.path}`)).replace(/<\//g, "<\\/")};`);
    lines.push(`  let models = ${JSON.stringify(models.slice(0, 8).map(m => m.name)).replace(/<\//g, "<\\/")};`);
    lines.push(`</script>`);
    lines.push("");
    ...
    lines.push(`<!-- ${codeComment(id.name)} — Vue 3 app shell paired with /theme.css. -->`);
    lines.push(`<template>`);
    lines.push(`  <main :class="['${kebab}-app']">`);
    lines.push(`    <header><h1>{{ title }}</h1></header>`);
    ...
    lines.push(`const title = "${jsString(id.name)}";`);
    lines.push(`const routes = ${JSON.stringify(routes.slice(0, 12).map(r => `${r.method.toUpperCase()} ${r.path}`)).replace(/<\//g, "<\\/")};`);
    lines.push(`const models = ${JSON.stringify(models.slice(0, 8).map(m => m.name)).replace(/<\//g, "<\\/")};`);
    lines.push(`</script>`);
    lines.push("");
    ...
    lines.push(`/**`);
    lines.push(` * ${codeComment(id.name)} — App shell.`);
    lines.push(` *`);
    ...
    lines.push(`          <p className="${kebab}-tagline">${jsxText(id.description ?? "Generated by Axis Artifacts.")}</p>`);
    lines.push(`        </header>`);
    ...
    lines.push(`/**`);
    lines.push(` * ${codeComment(id.name)} — vanilla DOM app factory.`);
    lines.push(`  * Language: ${codeComment(id.primary_language)}`);
    lines.push(` */`);
    lines.push("");
...
export function generateDashboardWidget(ctx: ContextMap, files?: SourceFile[]): GeneratedFile {
  ...
  if (isReact) {
    ...
    for (const lang of languages.slice(0, 3)) {
      lines.push(`        <StatCard label="${htmlEscape(lang.name)}" value={\`\${${JSON.stringify(lang.loc_percent)}}%\`} />`);
    }
    ...
    lines.push(`// Dashboard widget for ${codeComment(id.name)}`);
    lines.push("");
  ...
  if (hotspots.length > 0) {
    ...
    for (const h of hotspots.slice(0, 10)) {
      lines.push(`// ${codeComment(h.path)} | ${h.inbound_count} in | ${h.outbound_count} out | risk ${h.risk_score.toFixed(2)}`);
    }
  }
  ...
  if (entryPoints.length > 0) {
    ...
    for (const ep of entryPoints) {
      lines.push(`// [${codeComment(ep.type)}] ${codeComment(ep.path)} — ${codeComment(ep.description)}`);
    }
  }
  ...
  if (routes.length > 0) {
    ...
    for (const [method, count] of [...methodCounts.entries()].sort((a, b) => b[1] - a[1])) {
      lines.push(`// ${codeComment(method)}: ${count} endpoints`);
    }
  }
  ...
  if (models.length > 0) {
    ...
    for (const m of models.slice(0, 10)) {
      lines.push(`// ${codeComment(m.name)} (${codeComment(m.kind)}, ${m.field_count} fields) — ${codeComment(m.source_file)}`);
    }
  }
  ...
  if (signals.patterns_detected.length > 0 || signals.separation_score > 0) {
    ...
    if (signals.patterns_detected.length > 0) {
      lines.push(`// Patterns: ${codeComment(signals.patterns_detected.join(", "))}`);
    }
    if (signals.layer_boundaries.length > 0) {
      ...
      for (const lb of signals.layer_boundaries.slice(0, 5)) {
        lines.push(`//   ${codeComment(lb.layer)} (${lb.directories.length} dirs)`);
      }
    }
  }
  ...
  if (files && files.length > 0) {
    ...
    if (configs.length > 0) {
      lines.push(`// Config files: ${codeComment(configs.map(c => c.path).join(", "))}`);
    }
  }
...
export function generateEmbedSnippet(ctx: ContextMap, files?: SourceFile[]): GeneratedFile {
  ...
  lines.push("/**");
  lines.push(` * Embeddable context snippet for ${codeComment(id.name)}`);
...
export function generateArtifactSpec(ctx: ContextMap, profile: RepoProfile, files?: SourceFile[]): GeneratedFile {
  ...
  lines.push(`# Artifact Specification — ${mdText(id.name)}`);
  lines.push("");
  ...
  lines.push(`|-------|-------|`);
  lines.push(`| Name | ${mdInline(id.name)} |`);
  lines.push(`| Type | ${mdInline(id.type)} |`);
  ...
  if (entryPoints.length > 0) {
    ...
    lines.push("|------|------|-------------|");
    for (const ep of entryPoints) {
      lines.push(`| \`${mdCellCode(ep.path)}\` | ${mdInline(ep.type)} | ${mdInline(ep.description)} |`);
    }
  } else {
  ...
  lines.push("");
  lines.push(`1. **Component artifacts** should use ${mdText(frameworks[0] ?? id.primary_language)} conventions`);
  ...
  if (deps.length > 0) {
    for (const d of deps) {
      lines.push(`- \`${mdCode(d.name)}\` @ ${mdText(d.version)}`);
    }
  } else {
  ...
  if (files && files.length > 0) {
    ...
    if (components.length > 0) {
      ...
      for (const c of components.slice(0, 10)) {
        ...
        if (exports.length > 0) {
          lines.push(`- \`${mdCode(c.path)}\`: ${mdText(exports.join(", "))}`);
        }
      }
      lines.push("");
    }
...
export function generatePrd(ctx: ContextMap, _profile: RepoProfile, files?: SourceFile[]): GeneratedFile {
  ...
  const lines: string[] = [];
  lines.push(`# ${mdText(id.name)} — Product Requirements`);
  lines.push("");
  lines.push(`> ${mdText(ai.project_summary || id.description || `A ${id.type.replace(/_/g, " ")} built with ${frameworks.slice(0, 3).join(", ") || id.primary_language}.`)}`);
  lines.push("");
  ...
  if (ai.warnings.length > 0) {
    ...
    lines.push("");
    for (const w of ai.warnings.slice(0, 5)) lines.push(`- ${mdText(w)}`);
    lines.push("");
  }
  ...
  if (entryPoints.length > 0) {
    lines.push(`- Keep ${entryPoints.length} entry point${entryPoints.length === 1 ? "" : "s"} (${mdText(entryPoints.slice(0, 3).map(e => e.path).join(", "))}${entryPoints.length > 3 ? ", …" : ""}) trivially runnable: \`npm install && npm run dev\` (or equivalent).`);
  }
  ...
  if (routes.length > 0) {
    ...
    for (const [area, rs] of grouped) {
      lines.push(`### ${mdText(area)}`);
      for (const r of rs) {
        lines.push(`- As a user, I can call \`${mdCode(r.method.toUpperCase() + " " + r.path)}\` (source: \`${mdCode(r.source_file)}\`) and receive a successful, deterministic response.`);
      }
      lines.push("");
    }
  }
  ...
  lines.push("");
  lines.push(`- Primary language: **${mdText(id.primary_language)}**. New code follows the existing strict-mode / lint conventions.`);
  if (frameworks.length > 0) lines.push(`- Frameworks: ${frameworks.map(f => `**${mdText(f)}**`).join(", ")}. Stay on these unless an RFC is opened.`);
  if (ctx.detection.package_managers.length > 0) lines.push(`- Package manager: **${mdText(ctx.detection.package_managers[0])}** (lockfile is authoritative — commit it).`);
  if (ctx.detection.deployment_target) lines.push(`- Deployment target: **${mdText(ctx.detection.deployment_target)}**.`);
  if (ai.conventions.length > 0) {
    lines.push("");
    lines.push("**Engineering conventions:**");
    for (const c of ai.conventions.slice(0, 6)) lines.push(`- ${mdText(c)}`);
  }
  lines.push("");
...
export function generateDesignDoc(ctx: ContextMap, profile: RepoProfile, files?: SourceFile[]): GeneratedFile {
  ...
  lines.push("");
  lines.push(`Architecture, data flow, and UX decisions for ${mdText(id.name)}. Use this as the source of truth when proposing changes that cross module boundaries.`);
  lines.push("");

  lines.push("## Tech Stack");
  lines.push("");
  lines.push(`- **Language**: ${mdText(id.primary_language)}`);
  if (frameworks.length > 0) lines.push(`- **Frameworks**: ${mdText(frameworks.join(", "))}`);
  if (ctx.detection.build_tools.length > 0) lines.push(`- **Build tools**: ${mdText(ctx.detection.build_tools.join(", "))}`);
  if (ctx.detection.test_frameworks.length > 0) lines.push(`- **Test runner**: ${mdText(ctx.detection.test_frameworks.join(", "))}`);
  if (ctx.detection.package_managers.length > 0) lines.push(`- **Package manager**: ${mdText(ctx.detection.package_managers.join(", "))}`);
  if (ctx.detection.ci_platform) lines.push(`- **CI**: ${mdText(ctx.detection.ci_platform)}`);
  if (ctx.detection.deployment_target) lines.push(`- **Deploy target**: ${mdText(ctx.detection.deployment_target)}`);
  lines.push("");
  ...
  if (arch.patterns_detected.length > 0 || arch.layer_boundaries.length > 0) {
    ...
    if (arch.patterns_detected.length > 0) {
      lines.push("**Patterns detected:**");
      for (const p of arch.patterns_detected) lines.push(`- ${mdText(p)}`);
      lines.push("");
    }
  ...
  if (ctx.ai_context.conventions.length > 0) {
    for (const c of ctx.ai_context.conventions.slice(0, 6)) lines.push(`- ${mdText(c)}`);
  }
  if (ctx.ai_context.conventions.length === 0) {
...
export function generateTasksMd(ctx: ContextMap, profile: RepoProfile, files?: SourceFile[]): GeneratedFile {
  ...
  const lines: string[] = [];
  lines.push(`# ${mdText(id.name)} — Implementation Tasks`);
  lines.push("");
  ...
  if (models.length > 0) {
    ...
    for (const m of models.slice(0, 6)) {
      lines.push(`  - \`${mdCode(m.name)}\` (${mdText(m.kind)}, ${m.field_count} field${m.field_count === 1 ? "" : "s"}, \`${mdCode(m.source_file)}\`)`);
    }
  } else {
  ...
  if (routes.length > 0) {
    ...
    for (const r of routes.slice(0, 8)) {
      lines.push(`  - \`${mdCode(r.method.toUpperCase() + " " + r.path)}\` — \`${mdCode(r.source_file)}\``);
    }
...
export function generateContextMd(ctx: ContextMap, profile: RepoProfile, _files?: SourceFile[]): GeneratedFile {
  ...
  const lines: string[] = [];
  lines.push(`# ${mdText(id.name)} — Session Context`);
  lines.push("");
  ...
  if (ctx.detection.test_frameworks.length > 0) {
    lines.push(`- Test runner configured: ${mdText(ctx.detection.test_frameworks.join(", "))}`);
  }
  if (ctx.detection.ci_platform) {
    lines.push(`- CI pipeline live on ${mdText(ctx.detection.ci_platform)}`);
  }
  if (ctx.detection.deployment_target) {
    lines.push(`- Deployment target: ${mdText(ctx.detection.deployment_target)}`);
  }
  lines.push("");
  ...
  if (warnings.length > 0) {
    ...
    lines.push("");
    for (const w of warnings.slice(0, 6)) lines.push(`- ${mdText(w)}`);
    lines.push("");
  }
  ...
  if (entryPoints.length > 0) {
    for (const e of entryPoints.slice(0, 5)) {
      lines.push(`- \`${mdCode(e.path)}\` — ${mdText(e.description || e.type)}`);
    }
  }
  if (entryPoints.length === 0) {
  ...
  if (conventions.length > 0) {
    ...
    lines.push("");
    for (const c of conventions.slice(0, 6)) lines.push(`- ${mdText(c)}`);
    lines.push("");
  }
...
export function generateCapabilityMap(
  ...
  lines.push(`# Generated for ${escapeYamlString(ctx.project_identity.name)} (snapshot ${escapeYamlString(ctx.snapshot_id)}).`);
  lines.push(`# Generated at: ${ctx.generated_at}`);
  lines.push("");
  lines.push("meta:");
  lines.push(`  project: "${escapeYamlString(ctx.project_identity.name)}"`);
  lines.push(`  snapshot_id: "${escapeYamlString(ctx.snapshot_id)}"`);
  lines.push(`  generated_at: "${escapeYamlString(ctx.generated_at)}"`);
